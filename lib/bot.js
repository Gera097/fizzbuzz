...
bot.on("message", (msg) => {
    ...
    {
        const resp = ExplorerController.getExplorersNamesByMission(msg.text);
        resp.forEach(explorer => {
            bot.sendMessage(chatId, `El alumno ${explorer} pertenece a la misión ${msg.text}`);
        });
    }
    else {
